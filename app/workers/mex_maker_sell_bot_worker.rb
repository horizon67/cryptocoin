class MexMakerSellBotWorker
  ...
  def perform(*args)
    ...
    # 新規注文
    if orderable?(bitmex)
      bitmex.limit_sell(Settings.config.mex_maker_bot.amount, mex_ticker[:last].to_f)
    end
  end
  ...
  def cancel_order(bitmex)
    bitmex.get_open_orders.each do |order|
      bitmex.cancel_order(order[:orderID], order[:clOrdID]) if (order[:timestamp].in_time_zone('Tokyo') + 2.minutes) < Time.zone.now
    end
  end
end
